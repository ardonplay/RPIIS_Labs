...
void Set::SymmetricDifference(Set &first_set, Set &second_set) {
    this->clear();
    this->setSetName("(" + first_set.getSetName() + "△" + second_set.getSetName() + ")");
    Set diff_First_and_Second;
    diff_First_and_Second.difference(first_set, second_set);
    Set diff_Second_and_First;
    diff_Second_and_First.difference(second_set, first_set);
    Set result;
    result.unite(diff_First_and_Second,diff_Second_and_First);
    this->main_container.reserve(result.main_container.size());
    for (auto i: result.main_container) {
        this->push_back(i);
    }
}